...
| 4.4 | `ValidateJWTSVID` unary RPC | implemented | `internal/agent/workloadapi/server.go` `ValidateJWTSVID`; the agent fetches the JWKS from the control plane (`GET /v1/jwt/bundle`) on every call and verifies the token against it. An agent-side JWKS cache to avoid that per-call fetch is on the roadmap |
